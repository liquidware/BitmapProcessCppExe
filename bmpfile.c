...
void bmp_create_from_file(const char *filename, bmpfile_t *bmp, uint8_t *buff) {
	FILE *fin;
	fin = fopen(filename,"rb+");
	if(fin==NULL)
	{
	  printf("Error opening %s bmp file", filename);
	  exit(0);
	}
    fread(&bmp->magic,sizeof(bmp->magic),1,fin);
    fread(&bmp->header,sizeof(bmp->header),1,fin);
    fread(&bmp->dib,sizeof(bmp->dib),1,fin);
    printf("Reading image '%s' width %u, height %u\n",filename, bmp->dib.width, bmp->dib.height);

    int imsize = bmp->dib.width * bmp->dib.height * 3;
    //buff = (uint8_t*)malloc(imsize);

    fseek(fin, bmp->header.offset, SEEK_SET);
    fread(buff,imsize,1,fin);
}
